...
static void Get4points(
	...
	static volatile int16	i, j;
	static volatile AKFLOAT temp;
	static volatile AKFLOAT d;

	static  AKFVEC	dv[AKFS_HBUF_SIZE];
	static volatile AKFVEC	cross = {{0, 0, 0}};
	static volatile AKFVEC	tempv = {{0, 0, 0}};
...
int16 AKFS_AOC(				/*!< (o) : calibration success(AKFS_SUCCESS), failure(AKFS_ERROR) */
	...
	static volatile int16	i, j;
	static volatile int16	num;
	static volatile AKFLOAT	tempf;
	static  AKFVEC	tempho;

	static  AKFVEC	fourpoints[4];
	...
	for (i = 0; i < 4; i++) {
		for (j = (i+1); j < 4; j++) {
			...
			mean.u.x=tempf;mean.u.y=haocv->hraoc;mean.u.z=AKFS_HR_TH;
			if ((tempf < haocv->hraoc) || (tempf < AKFS_HR_TH)) {
				return AKFS_ERROR;
				//***********************************************
				//	mean.u.x=10.0;mean.u.y=20.0;mean.u.z=30.0;
				//	*ho = mean;
				//	return AKFS_SUCCESS;
				/***********************************************/
			}
	...
	if (CheckInitFvec(&haocv->hobuf[AKFS_HOBUF_SIZE-1]) == 1) {
		return AKFS_ERROR;
	//***********************************************
	//	mean.u.x=4.0;mean.u.y=5.0;mean.u.z=6.0;
	//	*ho = mean;
	//	return AKFS_SUCCESS;
	/***********************************************/
	}
	...
	if ((var.u.x >= tempf) || (var.u.y >= tempf) || (var.u.z >= tempf)) {
		return AKFS_ERROR;
		//***********************************************
		//	mean.u.x=7.0;mean.u.y=8.0;mean.u.z=9.0;
		//	*ho = mean;
		//	return AKFS_SUCCESS;
		/***********************************************/
	}

	//mean.u.x=1.0;mean.u.y=2.0;mean.u.z=3.0;
	*ho = mean;
